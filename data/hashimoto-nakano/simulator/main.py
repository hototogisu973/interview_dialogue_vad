...
if __name__ == "__main__":
    ...
    for setting_description in setting_descriptions:
        ...
        if task_description_file:
            with open(
                os.path.join(test_config_dir, task_description_file), encoding="utf-8"
            ) as fp:
        ...
        if not prompt_template_file:
            ...
            sys.exit(1)
        with open(
            os.path.join(test_config_dir, prompt_template_file), encoding="utf-8"
        ) as fp:
        ...
        settings.append(
            {"prompt_template": prompt_template, "initial_aux_data": initial_aux_data}
        )
    ...
    for setting in settings:
        for temperature in temperatures:
            ...
            user_utterance = user_simulator.generate_next_user_utterance(
                result["system_utterance"]
            )
